...
import { ByteArrayInput, ByteArrayOutput, Nullish, Option, safeExecute, Unhandled, UUID } from "std"
...
describe("ArrayField", () => {
	it("expect update event", () => {
		...
		const subscription = graph.subscribeBoxUpdates(Propagation.Children, fooBox.address, mockCallback)
		graph.beginTransaction()
